...
__weak void HAL_IncMicroTick(void)
{
  uwTickMicro += (uint32_t)uwTickFreq;
//	uwTick += (uint32_t)1;
}
...
__weak uint32_t HAL_GetMicroTick(void)
{
  return uwTickMicro;
}
...
__weak void HAL_DelayMicro(uint32_t Delay)
{
  uint32_t tickstart = HAL_GetMicroTick();
  uint32_t wait = Delay;

  while ((HAL_GetMicroTick() - tickstart) < wait)
  {
  }
}
